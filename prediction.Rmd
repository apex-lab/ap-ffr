...
behav <- read.csv("data/AP_ABR_8-2-2020_Behavioral.csv") # bad subjects already removed


# remove subjects with no pitch scores
x_piano <- x_piano[!is.na(behav$ACC_Piano_ACC),]
x_complex <- x_complex[!is.na(behav$ACC_Piano_ACC),]
x_da <- x_da[!is.na(behav$ACC_Piano_ACC),]
behav <- behav[!is.na(behav$ACC_Piano_ACC),]
...
y_avg <- (36*y_piano + 40*y_sine)/(36 + 40) # weighted b/c different number of trials
```
...
# Can our effect be explained by musical or tonal language experience?

We remove non-musicians and tonal language speakers from the sample, and train a model on the restricted sample of non-tonal language speaking musicians. (We do this because our musicians with and without AP are matched in musical experience, by virtue of how we sampled subjects during recruiting, but tonal language experience may vary.)


```{r}
# remove non-musicians and tonal language speakers
idx <- (behav$Group != "NO") * (behav$Tonal == 0) 
idx <- as.logical(idx)
x_piano <- x_piano[idx,]
y_avg <- y_avg[idx]
n <- length(y_avg) # number of subjects left
```

Now we'll run our model on just this restricted sample of `r n` subjects.
...
p1 <- ggplot(r_vals_rest, aes(x = stimulus, y = r, fill = stimulus)) +
  geom_violin() + stat_summary(fun = mean, geom = "point") +
  theme_minimal() + guides(fill = FALSE) + 
  labs(x = "Sample", y = "Correlation Values from Cross-Validation Runs")
comparisons = list(c(1, 2))
p2 <- ggplot(r_vals_rest, aes(x = stimulus, y = z, fill = stimulus)) +
  geom_violin() + stat_summary(fun = mean, geom = "point") +
  stat_compare_means(method = "t.test", label =  "p.signif", comparisons=comparisons, paired = F) +
  theme_minimal() + guides(fill = FALSE) + 
  labs(x = "Sample", y = "Z-transformed Correlation Values")
grid.arrange(p1, p2, nrow = 1)
```
...
The restricted model acheives an average performance of $r$ = `r round(rmu, 2)`, still significantly better than chance ($t$(`r round(r_vs_z$parameter, 2)`) = `r round(r_vs_z$statistic, 2)`, $p$ = `r formatC(r_vs_z$p.value, format = "e", digits = 2)`). Moreover, its predictive accuracy actually _improves_ compared to the unrestricted model ($t$(`r round(r_vs_u$parameter, 2)`) = `r round(r_vs_u$statistic, 2)`, $p$ = `r formatC(r_vs_u$p.value, format = "e", digits = 2)`). This suggests that we aren't merely replicating known effects of musical and tonal language experience on the FFR and on AP ability, but the FFR predicts AP ability on its own.
